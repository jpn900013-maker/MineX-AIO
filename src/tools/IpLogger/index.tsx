...
export default function IpLogger() {
    ...
    useEffect(() => {
        const fetchVisitors = async () => {
            // Read directly from localStorage to get latest state without dependencies
            const saved = localStorage.getItem("minex-ip-logger");
            if (!saved) return;

            let currentLinks: TrackedLink[] = [];
            try {
            ...
            if (updated) {
                ...
                // Update selected link if open - use functional ID check
                setSelectedLink(prev => {
                    if (!prev) return null;
                    const u = newLinks.find(l => l.id === prev.id);
                    return u || prev;
                });
            }
        };

        const interval = setInterval(fetchVisitors, 8000); // 8s poll
    ...
    const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (event) => {
            ...
            if (!token) {
                toast({ title: "Login Required", description: "You must be logged in to create tracking links", variant: "destructive" });
                setLoading(false);
                return;
            }
            ...
            try {
                ...
                toast({
                    ...
                    description: err.message || "Could not save to backend. Database might be offline.",
                    variant: "destructive"
                });
                setLoading(false);
                return;
