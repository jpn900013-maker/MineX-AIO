...
import { Tv, Play, Copy, Check, RotateCcw, ShieldCheck, Plus } from "lucide-react";
...
export default function DisneyGenerator() {
    ...
    const fetchStock = async () => {
        try {
            const res = await fetch(`${API_URL}/api/generator/disney/stock`);
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_URL}/api/generator/disney/generate`, {
                method: 'POST',
            ...
            if (data.success) {
                ...
                toast({ title: "Account Generated!", description: "Enjoy your Disney+ account." });
            } else {
    ...
    const handleRestock = async () => {
        ...
        try {
            const token = localStorage.getItem('minex-token') || '';
            const res = await fetch(`${API_URL}/api/generator/disney/add`, {
                method: 'POST',
                headers: {
    ...
    return (
        <ToolPageLayout
            title="Disney+ Generator"
            description="Generate premium Disney+ accounts instantly"
            icon={Tv}
            category="Generator"
        >
            <div className="grid md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                    <div className="bg-muted/30 border rounded-xl p-8 flex flex-col items-center justify-center text-center space-y-6 min-h-[300px]">

                        {/* Stock Counter */}
                        <div className="flex items-center gap-2 bg-background/50 px-4 py-2 rounded-full border">
                            <ShieldCheck className="w-4 h-4 text-green-500" />
                            <span className="text-sm font-medium">Stock: {stock !== null ? stock : '...'}</span>
                        </div>
                        ...
                        {!account ? (
                            <div className="space-y-4">
                                ...
                                <p className="text-muted-foreground">Click below to generate a fresh Disney+ account</p>
                                <Button
                                    size="lg"
                ...
                <div className="space-y-6">
                    <div className="p-6 border rounded-xl bg-card">
                        <h3 className="font-semibold mb-4 flex items-center gap-2">
                            <Tv className="w-4 h-4" /> Service Info
                        </h3>
                    ...
                    <div className="p-6 border rounded-xl bg-card">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="font-semibold">Restock (Admin)</h3>
